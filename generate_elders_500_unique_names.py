...
# Base real names (we will expand these safely)
base_first_names = [
...
# Expand safely to 250 real names
...
with open("elders_500.csv", "w", newline="", encoding="utf-8") as f:
    writer = csv.writer(f)
    writer.writerow([
        "username","email","password","full_name","phone",
        "address_line1","address_line2","city","state","pincode",
        "preferred_language","age","emergency_contact_name",
        "emergency_contact_phone","health_support","communication_mode"
    ])

    for i in range(500):
        fn = first_names[i % 250]
        ln = last_names[(i * 7) % 250]  # shuffled mapping prevents repeats
...
print("✅ elders_500.csv generated with 500 REAL full names (no fake NameXX values)")
